#!/usr/bin/env python3
"""Build the native-distribution silencer-vs-negative GRCh38 benchmark.

For each chromosome, the usable paired count is the smaller positive/negative
...
def record_destinations(chromosome: str) -> tuple[str, ...]:
    ...
    if chromosome not in CHROMOSOME_FOLD:
        return ()
                ...
                for index in selections[class_name][chromosome]:
                    ...
                    manifest.writerow(
                        (class_name, fixed_split, f"chr{chromosome}",
                         CHROMOSOME_FOLD.get(chromosome, ""), index + 1, bed_chrom,
...
def main() -> None:
    ...
    quotas = {
        chromosome: min(
            len(sources["pos"][chromosome].bed_records),
            len(sources["neg"][chromosome].bed_records),
        )
        for chromosome in CHROMOSOMES
    }
    ...
    write_outputs(
        paths, sources, selections, quotas, target_count, args.seed, args.overwrite
    )
    training_count = sum(quotas[chromosome] for chromosome in TRAINING_CHROMOSOMES)
    print(
        f"Selected benchmark: {target_count:,} positives and "
        f"{target_count:,} negatives"
    )
