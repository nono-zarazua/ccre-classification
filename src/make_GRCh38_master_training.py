...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--chroms-dir",
        type=Path,
        default=repository / "data/processed/evn/GRCh38/chroms",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=repository / "data/processed/evn/GRCh38/training",
    )
    parser.add_argument(
        "--folds-dir",
        type=Path,
        default=repository / "data/processed/evn/GRCh38/folds",
    )
...
def source_paths(chroms_dir: Path, chromosome: str, class_name: str) -> tuple[Path, Path]:
    ...
    if class_name == "pos":
        stem = f"chrom{chromosome}_GRCh38_ELS"
    else:
        stem = f"neg1x_chrom{chromosome}_GRCh38_ELS"
